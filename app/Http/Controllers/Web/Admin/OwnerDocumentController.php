...
class OwnerDocumentController extends BaseController
{
    ...
    public function approveOwnerDocument(Request $request)
    {
        ...
        if ($status) {
            $title = trans('push_notifications.owner_approved');
            $body = trans('push_notifications.owner_approved_body');
            $push_data = ['notification_enum'=>PushEnums::OWNER_ACCOUNT_APPROVED];
            $socket_success_message = PushEnums::OWNER_ACCOUNT_APPROVED;
        } else {
            $title = trans('push_notifications.owner_declined_title');
            $body = trans('push_notifications.owner_declined_body');
            $push_data = ['notification_enum'=>PushEnums::OWNER_ACCOUNT_DECLINED];
            $socket_success_message = PushEnums::OWNER_ACCOUNT_DECLINED;
        }
